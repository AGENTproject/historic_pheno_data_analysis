---
title: An R Markdown document converted from "WBF-Agroscope_historic_wheat_data_analysis.ipynb"
output: html_document
---

# Agroscope historical phenotypic wheat data analysis
origin and version of the input file: https://urgi.versailles.inrae.fr/fairdom/data_files/39?version=16
...
```{r}
data_total <- preprocess("Phenotypic_historical_wheat_jul2023-1-1.xlsx", obs_col_types = c('numeric', rep('text', 5), rep('numeric', 4), rep('skip', 7)))
write_csv(data_total, "Agroscope_wheat.csv")
...
write_csv(quality_all,"Agroscope_wheat_data_quality.csv")
```
...
write_csv(BLUE_all,"Agroscope_wheat_BLUE_values.csv")
summary(BLUE_all)
```

```{r}
library(rmarkdown)
convert_ipynb("WBF-Agroscope_historic_wheat_data_analysis.ipynb")
```
