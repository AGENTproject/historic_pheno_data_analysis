---
title: An R Markdown document converted from "ICARDA_historic_barley_data_analysis.ipynb"
output: html_document
---

# ICARDA historical phenotypic barley data analysis
origin and version of the input file: https://urgi.versailles.inrae.fr/fairdom/data_files/334?version=9
...
```{r}
data_total <- preprocess("Phenotypic_historical_template - SPRING-BARLEY_v7.xlsx")
write_csv(data_total, "ICARDA_barley.csv")
...
write_csv(data_corrected_all, "ICARDA_barley_corrected.csv")
```
...
```{r}
data_corrected_PH_II <- data_corrected_PH_II |> filter(campaign!="2010-2011") |> remove_lonely(2) #debug asreml
...
write_csv(quality_all,"ICARDA_barley_data_quality.csv")
```
...
write_csv(BLUE_all,"ICARDA_barley_BLUE_values.csv")
summary(BLUE_all)
```

```{r}
library(rmarkdown)
convert_ipynb("ICARDA_historic_barley_data_analysis.ipynb")
```
